...
// collectorState is the part of the collector only its own goroutine
// touches. Rate and pause changes arrive as functions over it on the
// control channel, so nothing here is read from two goroutines at once.
...
// server.
func (c *Collector) Run(ctx context.Context, rate time.Duration) {
	...
	var prev *Snapshot
	tick := func() {
		cur, err := Collect(ctx, c.db)
		if err != nil {
			c.fail(err)
			return
		}
		if c.onSample != nil {
			c.onSample(Derive(prev, cur))
		}
		prev = cur
	}
...
func (c *Collector) SetRate(d time.Duration) {
...
func (c *Collector) SetPaused(v bool) {
...
func (c *Collector) send(apply func(*collectorState)) {
	select {
