...
// C3: CREATE OR ALTER needs SQL Server 2016 SP1 (13.0.4001) and is the only
// reason the floor is SP1, not RTM; everything else runs on RTM or is
// version-gated. Each unconditional site is pinned here. README.md § Supported
// SQL Server versions lists the same three.
//
// Scans string literals via the parser, since comments mention the keywords
// too.
func TestOnlyKnownSitesEmitCreateOrAlter(t *testing.T) {
	// Repo-relative. gosmo's site (procedure.go) is pinned by gosmo's own copy
	// of this test.
