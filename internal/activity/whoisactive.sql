/*********************************************************************************************
MODIFIED by goSSMS (https://github.com/radix29/gossms), 2026-08-07.

This file is Adam Machanic's sp_WhoIsActive, licensed GPL-3.0 (see
LICENSE.sp_whoisactive in this directory). It is a verbatim copy of the
procedure definition from the upstream sp_WhoIsActive.sql release script with
two changes:

  1. The leading SET-options batch and the CREATE PROC stub batch were removed,
     so what remains is one batch that sp_executesql can run.
  2. goSSMS rewrites the procedure declaration line below into a
     "create or alter procedure dbo.<name>" at install time, so the copy it
     installs in tempdb can be named usp_WhoIsActive. An sp_-prefixed name is
     resolved against master from any database, which makes tempdb DDL under
     that name hit master's copy instead.

No other change has been made. Upstream: https://github.com/amachanic/sp_whoisactive
*********************************************************************************************/
/*********************************************************************************************
