...
func (d *BackupDialog) HandleMouse(ev *tcell.EventMouse) bool {
	...
	// lands outside the dialog or arrives in progress mode — both of which
	// return below, before any widget sees the event, so its next press
	// would be swallowed as a continuation of the stale drag. Each returns false on ButtonNone, so
