...
// branch: Availability Groups, and under each group its Replicas, Databases and
// Listeners.
//
...
// instances, and SSMS shows the folder regardless.
...
type agView struct {
	...
	// unreachable names the primary that could not be connected to, if following
	// was attempted and failed.
	unreachable string
}

// resolveAGView looks up a group by name and, when the local instance is not its
// primary, re-reads it through a connection to the primary.
//
// A failure to reach the primary is deliberately not an error: the local view is
...
// when it came from the primary the user is connected to. Without it a
...
// (database, replica) into one entry per database, keeping the query's ordering:
// the first row seen fixes a database's position.
...
// agDatabaseLabel renders "testdb_1 (Synchronized)". Distinct states across
// replicas are all shown — "(Synchronized, Synchronizing)" — since collapsing
// them hides exactly the replica that is behind.
...
// loadServerChildren inserts and any label refresh must match.
