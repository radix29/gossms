...
func propPageSets(sc *db.ServerConn, d *PropDialog) map[string][]propPage {
	return map[string][]propPage{
		...
		"Database Audit Specification": databaseAuditSpecificationPropPages(sc, "HealthClinic", "HIPAA_spec"),
		"Database Scoped Credential":   databaseScopedCredentialPropPages(sc, "HealthClinic", "app_cred"),
		"Server Trigger":               serverTriggerPropPages(sc, "ddl_audit"),
		"Database Trigger":             databaseTriggerPropPages(sc, "HealthClinic", "ddl_audit"),
...
var pagesThatOnlyRead = []string{
	...
	// Same shape one scope down: ENABLE/DISABLE/DROP ... ON DATABASE are
	// Object Explorer commands, and the body is edited by scripting an ALTER.
	"Database Trigger/General", "Database Trigger/Definition",
...
var propPageConstructors = []string{
	...
	"databaseAuditSpecificationPropPages", "databaseTriggerPropPages",
	"databaseScopedCredentialPropPages",
