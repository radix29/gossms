...
// this first pass is O(script) on the UI goroutine per keystroke. This is the
// reference implementation the cache is differentially tested against after
// every edit, which is why it lives in a test file: nothing in the binary needs
// the O(script) answer, and removing it would leave prefix_cache_test.go
// nothing to check.
